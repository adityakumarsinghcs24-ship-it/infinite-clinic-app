...
from .models import Todo, Booking
...
def checkout(request):
    ...
    # 1. Extract data sent from Frontend
    # Expected data: { "items": [...], "totalPrice": 500 }
    cart_items = data.get('items', [])
    total_price = data.get('totalPrice', 0)

    # 2. Create a formatted string of items for the record
    # Example: "Blood Test, X-Ray"
    item_names = [item['name'] for item in cart_items]
    details_string = ", ".join(item_names)

    # 3. Create the Booking in Database
    booking = Booking.objects.create(
        user=user,
        booking_details=details_string,
        total_price=total_price,
        status="Confirmed"
    )

    # 4. Return Data for the "Thank You" Page
    return Response({
        "booking_id": booking.id,
        "user": user.username,
        "date": booking.created_at.strftime("%Y-%m-%d"),
        "items": details_string,
